-- Migration: Recreate email_templates with locale support (idempotent)
-- Run: wrangler d1 execute ticket-system --remote --file=./migrations/003_email_templates_locale_fix.sql

-- Drop leftover table from failed previous migration
DROP TABLE IF EXISTS email_templates_new;

-- Check if email_templates already has locale column by trying to select it
...
-- Step 2: Copy existing data (use INSERT OR IGNORE to handle duplicates)
INSERT OR IGNORE INTO email_templates_v2 (name, locale, subject, body)
  SELECT name, 'zh-TW', subject, body FROM email_templates;

-- Step 3: Drop old table and rename
DROP TABLE email_templates;
ALTER TABLE email_templates_v2 RENAME TO email_templates;

-- Step 4: Insert en and ja templates
